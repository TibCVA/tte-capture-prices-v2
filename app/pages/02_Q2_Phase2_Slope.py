...
def render() -> None:
    ...
    if RESULT_KEY not in st.session_state:
        restored_ok = False
        try:
            restored_ok = bool(restore_question_payload_with_latest_run_fallback("Q2", RESULT_KEY))
        except Exception:
            restored_ok = False
        if not restored_ok:
            st.info("Aucune analyse Q2 chargee. Lance un refresh global ou recharge un run combine valide.")
    payload = st.session_state.get(RESULT_KEY)
    if not payload:
        st.info("Etat Q2 vide: aucune donnee exploitable restauree.")
        return
    ...
    with tab_tech:
        ...
        outlier_rows = derive_outlier_flags_from_q2(slopes_hist)
        render_data_quality_panel(
            annual_df=annual,
            countries=[str(c) for c in bundle.selection.get("countries", [])],
            years=[int(y) for y in bundle.selection.get("years", [])],
            extra_country_year_df=outlier_rows if not outlier_rows.empty else None,
        )
        render_livrables_panel(
